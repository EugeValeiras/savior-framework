...
public class ABMJavaFramework {
	...
	public static void initialize() {
		...
		springLayout.putConstraint(SpringLayout.WEST, scrollPaneList, 10, SpringLayout.WEST, frame.getContentPane());
		springLayout.putConstraint(SpringLayout.SOUTH, scrollPaneList, -10, SpringLayout.SOUTH, frame.getContentPane());
		springLayout.putConstraint(SpringLayout.EAST, scrollPaneList, -10, SpringLayout.EAST, frame.getContentPane());
		springLayout.putConstraint(SpringLayout.NORTH, scrollPaneList, -100, SpringLayout.SOUTH, frame.getContentPane());
		frame.getContentPane().add(scrollPaneList);
		scrollPaneList.setViewportView(jList);
		...
		springLayout.putConstraint(SpringLayout.SOUTH, scrollPaneAtt, -50, SpringLayout.NORTH, scrollPaneList);
		...
		springLayout.putConstraint(SpringLayout.SOUTH, btnAgregar, -10, SpringLayout.NORTH, scrollPaneList);
	...
	// /////////////////Public Methods to be Override/////////////////////////////
	public void inicializarDefaultListModel(DefaultListModel<Object> objetos2) {
		// VACIO PORQUE LO TRAIGO INICIALIZADO POR DEFECTO SOLO PARA QUE LO
		// SOBREESCRIBAN
	}
	...
	private static void iniciarBusqueda(JList<Object> list) {
		_limpiarLista(list);
		try {
			...
			_lanzarPopUp("MIRARME ALGO ANDA MAL");
		} finally {
	...
	public void buscarObjetoALista(JList<Object> list) {
		...
		try {
			...
			// TODO BUSCAR OBJETO SIN LA PALABRA COMPLETA! -
		} catch (Exception e) {
			_lanzarPopUp(e);
		}
	}
	...
	private static Boolean _CompletarObjetoConDatos(Object newObj, Field[] fields, Boolean camposNoCompletos, Boolean lanzarPopUps) throws NoSuchMethodException, InstantiationException,
		...
		for (int i = 0; i < fields.length; i++) {
			FieldABM field = fields[i].getAnnotation(FieldABM.class);
			Validar validar = fields[i].getAnnotation(Validar.class);
			representationType representacion = field.representacion();
			...
			if (validar != null) {
				if (validar.abmValidator() == ABMJavaFrameworkValidator.NO_METHOD && validar.nombreMetodo().equals("NOMETHOD")) {
					fields[i].set(newObj, object);
				} else {
					_validarCampo(newObj, fields, i, validar, object);
				}
			} else {
				fields[i].set(newObj, object);
			}
	...
	private static void _validarCampo(Object newObj, Field[] fields, int i, Validar validar, Object object) throws IllegalAccessException {
		if (validar.abmValidator() == ABMJavaFrameworkValidator.NO_METHOD) {
			try {
				Method method = clase.getMethod(validar.nombreMetodo());
				if ((Boolean) method.invoke(newObj)) {
					fields[i].set(newObj, object);
				}
			} catch (SecurityException e) {
				_lanzarPopUp(e, "El metodo es PRIVADO");
			} catch (NoSuchMethodException e) {
				_lanzarPopUp(e, "EL nombre del metodo es incorrecto: " + validar.nombreMetodo());
			} catch (Exception e) {
				_lanzarPopUp(e);
			}
		} else if (validar.nombreMetodo().equals("NOMETHOD")) {
			if (_validacion(validar.abmValidator())) {
				fields[i].set(newObj, object);
			}
		}
	}

	private static boolean _validacion(ABMJavaFrameworkValidator abmValidator) {
		if (abmValidator == ABMJavaFrameworkValidator.SOLO_LETRAS) {
			return _ValidarSoloLetras();
		} else if (abmValidator == ABMJavaFrameworkValidator.SOLO_NUMEROS) {
			return _ValidarSoloNumeros();
		} else {
			_lanzarPopUp("ALGO ANDA MAL CON LA VALIDACION");
			return false;
		}
	}

	private static boolean _ValidarSoloLetras() {
		// TODO Auto-generated method stub
		System.out.println("ESTOY VALIDANDO SI SON SOLO LETRAS!");
		return true;
	}

	private static boolean _ValidarSoloNumeros() {
		// TODO Auto-generated method stub
		System.out.println("ESTOY VALIDANDO SI SON SOLO NUMEROS!!");
		return true;
	}
